...
import { getRealstate } from "../graphql/queries";
import { updateRealstate } from "../graphql/mutations";
...
export default function RealstateUpdateForm(props) {
  const {
    id: idProp,
    realstate: realstateModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = realstateRecord
      ? { ...initialValues, ...realstateRecord }
      : initialValues;
  ...
  const [realstateRecord, setRealstateRecord] =
    React.useState(realstateModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getRealstate.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRealstate
        : realstateModelProp;
      setRealstateRecord(record);
    };
    queryData();
  }, [idProp, realstateModelProp]);
  React.useEffect(resetStateValues, [realstateRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateRealstate.replaceAll("__typename", ""),
            variables: {
              input: {
                id: realstateRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RealstateUpdateForm")}
      {...rest}
    >
      <TextField
        label="Description"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || realstateModelProp)}
