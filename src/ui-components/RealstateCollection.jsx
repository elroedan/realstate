...
import { listRealstates } from "../graphql/queries";
...
export default function RealstateCollection(props) {
		...
		while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
			...
			const result = (
				await client.graphql({
					query: listRealstates.replaceAll("__typename", ""),
					variables,
				})
			).data.listRealstates;
	...
	return (
		<div>
			<Collection
				...
				{...getOverrideProps(overrides, "RealstateCollection")}
				{...rest}
			>
				{(item, index) => {
					if (loading) {
